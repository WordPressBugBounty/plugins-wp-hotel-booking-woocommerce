...
class RoomCartTotal {
	...
	protected function get_items_from_cart() {
		...
		foreach ( $cart_items as $cart_item_key => $cart_item ) {
			...
			$subtotal                 = wc_add_number_precision_deep( $cart_item['data']->get_price() * $item->quantity );

			/*** Custom ***/
			if ( isset( $cart_item['product_id'] ) && get_post_type($cart_item['product_id']) === 'hb_room' ) {
				$hb_optional_quantity_selected = $cart_item['hb_optional_quantity_selected'] ?? array();
				$extra_price = 0;
				foreach ($hb_optional_quantity_selected as $extra_item_id => $extra_item_status) {
					if ($extra_item_status === 'on') {
						$extra_item_price = get_post_meta($extra_item_id, 'tp_hb_extra_room_price', true);
						if (empty($extra_item_price)) {
							$extra_item_price = 0;
						}

						$extra_price += $extra_item_price * $cart_item['hb_optional_quantity'][$extra_item_id];
					}
				}

				$subtotal += wc_add_number_precision_deep($extra_price);
			}
	 ...
	 *
	 */
	protected function set_total( $key = 'total', $total = 0 ) {
	...
	*/
	...
	protected function calculate_discounts() {
		...
		// start get total coupon variable item
		$list_discount_item_variable = array();
		$total_discount_variable     = 0;

		foreach ( $cart_contents as $key => $value ) {
			if ( isset( $value['tour_variations'] ) ) {
				$tour_variations         = $value['tour_variations'];
				$tour_variations_options = $value['tour_variations_options'];
				$price                   = 0;

				foreach ( $tour_variations as $k_variations => $v_variations ) {
					foreach ( $v_variations as $k_attr => $v_attr ) {
						if ( isset( $v_attr->quantity ) ) {
							$price += $v_attr->quantity * (float) $tour_variations_options->$k_variations->variation_attr->$k_attr->price;
						} else {
							$price += (float) $tour_variations_options->$k_variations->variation_attr->$k_attr->price;
						}
					}
				}

				$total_discount_variable += $price; // total discount coupon by variable item cart.
				// Calculate discount for each item tour
				$price  *= ( 10 ** $num_decimals );
				$price_2 = 0;

				foreach ( $this->coupons as $coupon ) {
					if ( $coupon->get_discount_type() == 'percent' ) {
						$price_2 += floor( $price * ( (int) $coupon->get_amount() / 100 ) );
					}
				}
				$list_discount_item_variable[ $key ] = $price_2;
			}
		};

		$total_discount_variable *= ( 10 ** $num_decimals );

		$list_discount_variable = array();
		$flag_type              = false;
		...
		foreach ( $this->coupons as $coupon ) {
			...
			if ( $coupon->get_discount_type() == 'percent' ) {
				$flag_type                                     = true;
				$discount_variable                             = floor( $total_discount_variable * ( (int) $coupon->get_amount() / 100 ) );
				$list_discount_variable[ $coupon->get_code() ] = $discount_variable;
			}
		}

		$coupon_discount_amounts = $discounts->get_discounts_by_coupon( true );

		$discount_items = (array) $discounts->get_discounts_by_item( true );
		if ( $flag_type ) {

			foreach ( array_keys( $list_discount_variable + $coupon_discount_amounts ) as $key ) {
				$coupon_discount_amounts[ $key ] = ( isset( $list_discount_variable[ $key ] ) ? $list_discount_variable[ $key ] : 0 ) + ( isset( $coupon_discount_amounts[ $key ] ) ? $coupon_discount_amounts[ $key ] : 0 );
			}
			foreach ( array_keys( $list_discount_item_variable + $discount_items ) as $key ) {
				$discount_items[ $key ] = ( isset( $list_discount_item_variable[ $key ] ) ? $list_discount_item_variable[ $key ] : 0 ) + ( isset( $discount_items[ $key ] ) ? $discount_items[ $key ] : 0 );
			}
		}

		//end get total coupon variable item
		...
		$this->coupon_discount_totals     = $discount_items;
